        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-8">
          <div>
            <div className="flex items-center space-x-2 mb-4">
              ...
              <span className="text-xl font-bold text-brand-dark font-serif">
                {footer.logo}
              </span>
            </div>
            <p className="text-sm text-brand-text">{footer.tagline}</p>
          </div>

          {footer.sections.map((section, index) => (
            <div key={index}>
              <h4 className="font-bold text-[#5A0D29] mb-4">{section.title}</h4>
              <ul className="space-y-2">
                {section.links.map((link, linkIndex) => (
                  <li key={linkIndex}>
                    <a
                      href={link.href}
                      className="text-sm text-brand-text hover:text-[#5A0D29]"
                    >
                      {link.label}
                    </a>
                  </li>
