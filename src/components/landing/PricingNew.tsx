...
const PricingCard: React.FC<PricingCardProps> = ({
  ...
  const cardClasses = `p-8 rounded-2xl border ${
    isFeatured
      ? 'bg-[#771138] text-white border-[#771138]'
      : 'bg-white border-gray-200'
  }`;
  const buttonClasses = `w-full mt-8 py-3 px-6 font-bold rounded-full transition-all ${
    isFeatured
      ? 'bg-white text-[#5A0D29] hover:bg-gray-100'
      : 'bg-[#771138] text-white hover:bg-[#5A0D29]'
  }`;
  ...
  return (
    <div className={cardClasses}>
      ...
      <p className="mt-4">
        <span
          className={`text-4xl font-bold ${isFeatured ? 'text-white' : 'text-[#5A0D29]'}`}
        >
          {price !== 'Contact' && '€'}
          {price}
        </span>
      ...
      <ul className="mt-6 space-y-4">
        {features.map((feature, index) => (
          <li key={index} className="flex items-center">
            <CheckIcon
              className={`h-5 w-5 mr-3 flex-shrink-0 ${isFeatured ? 'text-[#E9B046]' : 'text-[#5A0D29]'}`}
            />
            <span>{feature}</span>
          </li>
        ))}
